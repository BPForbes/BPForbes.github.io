...
\textbf{P2 (static-site only):} Parity features ship inside the Vite bundle. No \texttt{VITE\_QPU\_API\_URL}, no Express/Fastify host, no server sessions. Long work uses \textbf{Web Workers}; persistence uses \textbf{localStorage}, \textbf{sessionStorage}, \textbf{IndexedDB}, and \textbf{Blob} file I/O (patterns already used by the catalog and \texttt{downloadQpucirSource}).
...
\noindent\textbf{Out of scope for this plan:} HTTP session APIs, server-side mutexes, multi-tenant hosting, server GC. Cross-tab sharing is optional via \texttt{BroadcastChannel} + exported \texttt{qpu-session} files, not a backend.
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Not used:} application server, sidecar API, server-side sim sessions.
\end{itemize}
...
%=============================================================================
\section{Client API (in-browser, no HTTP)}
%=============================================================================

All ``API'' surface area is \textbf{TypeScript functions and types} imported by React components and tests. There is no network boundary inside the parity plan.
...
Sessions live in a \texttt{Map} inside a React context provider (\texttt{SimulationProvider}) or module singleton; they are \textbf{not} addressable over HTTP.

\subsection{Result types and error codes (no HTTP)}
...
Parity is achievable \textbf{entirely on GitHub Pages}: cycle-capable executor with destructive-pop $\HS$ (Python semantics); math layers extending marginal $\rho_k$, Euler ket, and triple-integral $\langle\rho\rangle$; \texttt{SimulationSession} + \texttt{QpuResult} client API; versioned file exports; Web Workers for heavy correction. \textbf{No application backend} is required; \textbf{GitHub Actions} automates build, test, deploy, and optional Python cross-validation scripts. Verified gaps in \texttt{src/}: HilbertSpace, cycle runtime, checkpoints, noise, \texttt{SimulationSession}, correction Worker, and CI test gate (\texttt{deploy-pages.yml} builds without \texttt{npm test}).
