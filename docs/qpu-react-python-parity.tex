...
\begin{tikzpicture}[
  ...
  \draw[->] (ui) -- (sim);
  \draw[->] (sim.north) to[bend left=20] (store.north);
...
export async function fetchStateVector(sessionId: string): Promise<QpuResult<Complex[]>> {
  return qpuFetch<{ amplitudes: Complex[] }>(`/v1/sessions/${sessionId}/state`)
    .then((r) => (r.ok ? { ok: true, value: r.value.amplitudes } : r));
}
