...
\noindent\textbf{Optional (not shipped on Pages):} a sidecar HTTP service deployable separately; the static client talks to it only through \texttt{src/simulator/client/qpuFetchClient.ts}. \textbf{Out of scope:} multi-tenant hosting, mandatory backend, bundling Express into \texttt{dist/}.
...
\subsection{Python $\leftrightarrow$ React discrepancies (verified)}

\begin{center}
\small
\begin{tabular}{@{}p{0.5cm}p{3.2cm}p{9.8cm}@{}}
\toprule
 & Topic & Status \\
\midrule
1 & Qubit bit index & React: MSB-left, \texttt{1 << (n-1-k)} (\texttt{operations.ts}). Python \texttt{StateVector}: \texttt{1 << k}. Cross-impl tests must map wire labels explicitly; do not assume identical $k$. \\
2 & \texttt{extract\_qubit} & Python sums amplitudes (product-state approx). React uses full state vector + \texttt{marginalRho} (partial trace). TS is authoritative for Bloch display. \\
3 & Controlled gates & React: unitary \texttt{applyControlledX} over basis indices. Python: classical threshold \texttt{abs(c\_state[1])>0.5} on register slices---no superposition-controlled entanglement. Cycle-mode parity must choose which model to replicate. \\
4 & \texttt{HilbertSpace.copy} & Python has \texttt{copy()}; required for checkpoints. Added to TS interface above. \\
5 & \texttt{bb84.py}, \texttt{xmss.py} & \textbf{Out of scope} for this parity target (future phase). \\
6 & Section hooks & Python \texttt{GLOBALPROLOGUE}/\texttt{MAINPROLOGUE}/\ldots not in React compiler yet; cycle mode must add or emulate. \\
7 & \texttt{ClockFrame} stack & Python nested subprocess cycle bases; spec cycle mode must document absolute $c = \mathrm{base}+\mathrm{local}$. \\
8 & Protocol syntax & \textbf{Already in React:} \texttt{readProtocolLines} supports \texttt{\textbackslash} continuation, \texttt{\#}, \texttt{/* */} (\texttt{qpuAst.ts}). \\
9 & \texttt{embed\_registers} & Python register-padding helper; relevant only if \texttt{RegisterBank} cycle mode mirrors Python register model. \\
10 & Memory snapshot & Python \texttt{generate\_complete\_memory\_snapshot}; optional LLM context beyond \texttt{hilbert.view()}. \\
11 & RNG & React today: \texttt{Math.random()} in \texttt{measureQubit} (injectable for tests). Planned noise: prefer \texttt{crypto.getRandomValues()} or seeded PRNG. \\
12 & Ket display precision & \textbf{Already aligned:} \texttt{formatComplex} uses 3 decimals (\texttt{complex.ts}), matching Python \texttt{\{:.3f\}}. \\
13 & Device table RNG & Python \texttt{np.random} at init; parity tests should use fixed \texttt{deviceTable} fixture. \\
14 & \texttt{sp} token & \textbf{Diverges today:} Python random superposition at QPU init; React applies \texttt{H} via \texttt{applyStartState} ($\approx\ket{+}$ from $\ket{0}$), deterministic. \\
15 & \texttt{FREE} & React compiler logs only; no memory/$\HS$ removal yet (\texttt{qpuAst.ts}). \\
16 & Frequency fingerprint & Python \texttt{compare\_frequency\_fingerprints}; out of scope until device UI lands. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[label=\textbf{Phase \Roman*.}]
  ...
  \item \texttt{SimulationSession} facade + \texttt{QpuResult} errors; wire \texttt{App.tsx} / Module Lab.
  \item \texttt{RegisterBank}, noise, device table; extend particle math kernels.
  \item \texttt{qpucir v2}, \texttt{qpucirMigrate.ts}, catalog v2; \texttt{qpu-session} export/import.
  \item \texttt{correctionWorker.ts} for long jobs; optional IndexedDB session cache.
  \item \texttt{ci.yml} (\texttt{npm test} on PR); optional \texttt{parity-python.yml} for scripted cross-impl checks.
  \item \texttt{qpuFetchClient.ts} + optional sidecar deploy (not bundled into Pages).
...
Parity is achievable on \textbf{GitHub Pages} by default: cycle executor, $\HS$ log, particle math, \texttt{SimulationSession}, Workers, and file exports. An \textbf{optional} sidecar extends the same \texttt{QpuClient} types via \texttt{fetch}. \textbf{GitHub Actions} automates CI/deploy. Verified gaps in \texttt{src/}: HilbertSpace, cycle runtime, checkpoints, noise, \texttt{SimulationSession}, \texttt{qpuFetchClient}, correction Worker.
